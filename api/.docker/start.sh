...
##
## Got any issues or want something added or tweaked ask Lawrence <lawrence@cherone.co.uk>

# exit on error and no undefined vars
set -eu
...
main () {
    ...
    if [ "$(docker ps -aq -f name=projex)" ] && [ "$FORCE_INSTALL" = false ]; then

        echo -e >&2 "\033[1;31mProjex container(s) already exist!\033[0m"
